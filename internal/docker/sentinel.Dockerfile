...
# Build on Alpine so the resulting binary is musl-linked and runs unchanged
# on the Alpine-based ${BASE_IMAGE} runtime. A glibc builder (e.g.
# debian:bookworm-slim) compiles against libc.so.6 / libm.so.6 — those names
# don't exist on Alpine, and the dynamic loader fails with "symbol not found"
# relocations (backtrace_symbols_fd, __getauxval, fcntl64, …) at startup.
FROM alpine:3.21 AS builder

ARG REDIS_VERSION

RUN apk add --no-cache build-base curl linux-headers openssl-dev pkgconfig
...
FROM ${BASE_IMAGE}
# The Alpine redis image ships /usr/local/bin/redis-sentinel as a symlink to
# redis-server (dispatch is via argv[0]). Docker's COPY follows symlinks at
# the destination, so without this rm the next COPY would overwrite the
# real redis-server binary instead of replacing the sentinel symlink. We
# want the upstream redis-server intact; only redis-sentinel gets swapped
# for the tilt-patched one we just built.
RUN rm -f /usr/local/bin/redis-sentinel
