...
const (
	...
	// New v8 instances enter with a deprioritized replica-priority (higher
	// number than the v6 default), so any failover triggered from this state
	// stays within the v6 subset and leaves replication intact.
	StateMixedVersions State = "MixedVersions"
	// StateV8Prioritized: v8 replicas have been swapped to a low replica-priority
	// number and v6 left at the default. The next sentinel failover will
	// promote a v8 candidate, breaking replication between v6 and v8.
	StateV8Prioritized State = "V8Prioritized"
